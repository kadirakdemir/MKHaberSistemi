...
using MKHaberSistemi.Web.Models;
...
namespace MKHaberSistemi.Web.Areas.Admin.Controllers
{
    ...
    //[Authorize]
    //[RouteArea("Admin")]
    //[RoutePrefix("UserLoginOperations")]
            ...
            if (ModelState.IsValid)
            {
                ...
                if (userCount==0)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        //await UserManager.SendEmailAsync(user.Id, "Hesabını onayla", "Lütfen hesabınızı tıklayarak onaylayın. <a href=\"" + callbackUrl + "\">here</a>");

                        //   await _userManager.AddClaimAsync(user, new Claim("KullaniciAdi", user.KullaniciAdi));
